...
from game_variables import ID_AUTOGENERATE_MINI_GAME, prop_quantity_by_rarity
...
async def build_allowed_actions(
    ...
    if not recent_chat_action:
        for row in (observation.get("chat_games") or [])[:3]:
            # Join only public/global chat games (autogenerated by bot), skip private/user games.
            if int(row.get("owner_idpk", 0) or 0) != int(ID_AUTOGENERATE_MINI_GAME):
                continue
